...
class ViewDriversCompanyScreen(Screen):
    ...
    def load_drivers(self):
        """
        Carga los conductores de la compañía actual
        """
        try:
            ...
            # Verificar si es admin (administrador de compañía) o company (compañía directamente)
            ...
            # Obtener company_id dependiendo del rol
            if user_role == 'admin':
                company_id = current_user.get('company_id')
                print(f"Admin - Company ID: {company_id}")
            elif user_role == 'company':
                company_id = current_user.get('id')  # Para company, el ID es el company_id
    ...
    def create_driver_widget(self, driver):
        """
        Crea un widget simple para mostrar información básica de un conductor
        """
        # Contenedor principal con fondo
        container = BoxLayout(
            orientation='vertical',
            size_hint_y=None,
            height=dp(120),  # Aumentamos la altura para el botón
            padding=dp(15),
            spacing=dp(5)
        )
    ...
    def go_back(self):
        """
        Regresa al dashboard apropiado según el rol del usuario
        """
        try:
            current_user = App.get_running_app().current_user
            if current_user:
